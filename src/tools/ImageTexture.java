...
public class ImageTexture implements ISampler {
  ...
  public Color getColor(Vec2 at) {
    ...
    return applyGamma(divide(color, componentScale), 2.2);
  }

  public Color applyGamma(Color color, double gamma) {
    double r = Math.pow(color.r(), gamma);
    double g = Math.pow(color.g(), gamma);
    double b = Math.pow(color.b(), gamma);
