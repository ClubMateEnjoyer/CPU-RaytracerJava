...
public class ModelLoader {
    ...
    // lädt ein obj-model mit material aus der mtl
    public static Group loadOBJModel(String filename) {
        ...
        if (meshdatas.isEmpty()) {
            System.out.println("there is no meshdata in <" + filename + ">, aborting...");
            return null;
        }
    ...
    // lädt ein obj-model mit eignem material

    public static Group loadOBJModel(String filename, IMaterial overrideMaterial) {
        List<tools.Wavefront.MeshData> meshdatas = tools.Wavefront.loadMeshData(filename);
        if (meshdatas.isEmpty()) {
            System.out.println("there is no meshdata in <" + filename + ">, aborting...");
            return null;
        ...
        for (TriangleData tri : wf_triangles) {
            ...
            Vertex v1 = tri.v1();
            Vertex v2 = tri.v2();
            triangleGroup.addShape(new Triangle(v0, v1, v2, overrideMaterial));
        }
        return triangleGroup;
    }
}